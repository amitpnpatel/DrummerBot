...
class Beater
{
  ...
  public:
    ...
    void updatecounter() {
      if (state) {
        ...
        if (currentStepNumber < numberOfStepInSwing / 2) {
           currentTimeCounter = startStepTime - (((startStepTime - currentbeatAmplitude)*(currentStepNumber % (numberOfStepInSwing / 2))) / (numberOfStepInSwing / 2));
    ...
    void setBeat(int beatToLoad) {
      ...
      upSwingTime = (SWING_TIME / (numberOfStepInSwing / 2));
      currentTimeCounter = startStepTime;
      currentPlayingBeat = beatToLoad;
    }
};
int calibrationForBeaterOne = 4;
int calibrationForBeaterTwo = 5;
Beater beater1 = Beater(0, 13, 12, 11, 10, calibrationForBeaterOne); //define beater
Beater beater2 = Beater(1, 9, 8, 7, 6 , calibrationForBeaterTwo);
